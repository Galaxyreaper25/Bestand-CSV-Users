...
$HomeFolderShare = "E:\Shares\HomeFolders"
$UserProfilesShare = "E:\Shares\UserFolders"
...
mkdir -Path $HomeFolderShare
New-SmbShare -Name "HomeFolders$" -Path $HomeFolderShare -FullAccess $DomainAdmins -ChangeAccess $DomainUsers
...
# Een while loop die interactief gebruikt heeft met de gebruikers, en vraagd om de specifieke locatie om de NTFS rechten in te stellen.
write-host -foregroundColor green -object "De loop om de NTFS rechten wordt afgespeeld en hier heeft u interactie mee!"
while(($INPUT = Read-Host -Prompt "Push a key for NTFS configuration!") -ne "Q"){

    $Path = Read-Host -Prompt 'Type here your file location for the NTFS rights!'
    $PathDL = read-Host -prompt 'Type here your DL path location'

    $acl.SetAccessRuleProtection($true,$false)
    $everyone = New-Object System.Security.Principal.NTAccount("Everyone")
    $acl.PurgeAccessRules($everyone)
    $acl | set-Acl E:\Shares\BedrijfsData\$Path

    $ACL = Get-Acl E:\Shares\BedrijfsData\$Path
    ...
    $ACL | Set-Acl E:\Shares\BedrijfsData\$Path    #DL_Verkoop_RW

    $ACL = Get-Acl E:\Shares\BedrijfsData\$Path
    $AccessRules = New-Object system.security.AccessControl.FileSystemAccessRule("DL_$PathDL-Share_R","Read","Allow")
    $ACL.SetAccessRule($AccessRules)
    $ACL | Set-Acl E:\Shares\BedrijfsData\$Path

    $ACL = Get-Acl E:\Shares\BedrijfsData\$Path
    $AccessRules = New-Object system.security.AccessControl.FileSystemAccessRule("DL_$PathDL-Share_RW","Readandexecute,write","Allow")
    $ACL.SetAccessRule($AccessRules)
    $ACL | Set-Acl E:\Shares\BedrijfsData\$Path
        
}

# Commando's om de users toe te voegen
...
Write-Warning -Message "De computer zal na 10 seconden herstarten. Start hierna scrips 5!" 
